...
export class VideoTrackConstraints {
  ...
  constructor(source) {
     ...
     * @instance
     */
    this.resolution = new Owt.Base.Resolution(480, 640);   //fixed new browser open camera failed error
...
export class MediaStreamFactory {
  ...
  static createMediaStream(constraints) {
    console.log("enter =====================>>>>> resolution inner createMediaStream:" +  JSON.stringify(constraints));
    ...
    if (typeof constraints.video === 'object') {
      ...
      if (typeof constraints.video.deviceId === 'string') {
        ...
        console.log("odeviceID is string");
        // mediaConstraints.video = eval(constraints.video);
      }
      else{
        mediaConstraints.video = eval(constraints.video);
      }
    ...
    console.log("=====================>>>>> resolution inner createMediaStream:" +  JSON.stringify(mediaConstraints));

    // "{ audio: true, video: { facingMode: "user" } }"
    // mediaConstraints.video.width = 480;
    // mediaConstraints.video.height = 640;
    // mediaConstraints.audio = true;
    if (isVideoConstrainsForScreenCast(constraints)) {
      console.log("screen cast")
      ...
      //     console.log("start stream ok")
      // }).catch(function(err){
      //     console.log("erro msasge:" + JSON.stringify(err))
      // });
    }
  }
}
